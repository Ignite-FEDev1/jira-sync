...
export class SyncOrchestrator {
  ...
  async execute(options: SyncOptions): Promise<SyncSummary> {
    ...
    try {
      ...
      if (membershipProfile) {
        this.logger.info(
          `MEMBERSHIP 프로필 로드 완료: link_field=${membershipProfile.linkField}, 에픽 필터=[HM] 접두사 기반`
        );
      }
  ...
  private async classifyTicketsByTargetProject(
    ...
    for (const ticket of fehgTickets) {
      ...
      if (targetProjects.includes('AUTOWAY')) {
        ...
        // 에픽 이름이 [GW] 또는 [GW-QA지원]으로 시작하면 AUTOWAY 동기화 대상
        const isGwEpic =
  ...
  private async determineTargetProjectsForEpic(
    ...
    try {
      ...
      if (
        epicSummary.startsWith('[GW]') ||
        epicSummary.startsWith('[GW-QA지원]')
      ) {
