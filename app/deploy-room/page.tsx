...
export default function DeployRoomListPage() {
  ...
  const [sessions, setSessions] = useState<DeployRoomSessionWithProgress[]>([]);
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
            {sessions.map((session) => {
              ...
              const done = session.doneCount ?? 0;
                        ...
                        <div className="flex items-start justify-between gap-2">
                          <div className="flex items-center gap-1.5 flex-wrap min-w-0">
                            ...
                            {session.deployType && DEPLOY_TYPE_CHIP[session.deployType] && (
                              <span
                                className={`text-[10px] px-1.5 py-0.5 rounded font-semibold shrink-0 ${DEPLOY_TYPE_CHIP[session.deployType].style}`}
                              >
                      ...
                      <CardContent className="space-y-3">
                        <div>
                          <div className="flex items-baseline justify-between mb-1.5">
                            ...
                            <span className="text-xs tabular-nums text-muted-foreground">
                              <span
                                className={`font-bold text-base ${isComplete ? 'text-emerald-600' : 'text-foreground'}`}
                              >
                                {pct}
                              </span>
                        ...
                        <div className="flex items-end justify-between">
                          <div className="text-xs text-muted-foreground space-y-1">
                            {session.createdBy && <div>담당: {session.createdBy}</div>}
                          </div>
                          <button
                            type="button"
