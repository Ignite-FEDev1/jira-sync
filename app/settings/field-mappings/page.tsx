...
export default function FieldMappingsPage() {
  ...
  const fetchData = useCallback(async () => {
    const [projectsRes, profilesRes, mappingsRes, statusRes, workflowRes, teamsRes] = await Promise.all([
  ...
  const handleEdit = async (profile: SyncProfile) => {
    ...
    // HMG 대상인 경우 link_field, source_link_field 로드
    ...
    const sortKey = (arr: { sourceField?: string; sourceStatusId?: string }[]) =>
      JSON.stringify([...arr].sort((a, b) => JSON.stringify(a).localeCompare(JSON.stringify(b))));
    snapshotRef.current = {
