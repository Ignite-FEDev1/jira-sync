...
export default function AdminHolidaysPage() {
  ...
  const grouped = useMemo(() => {
    const acc = filtered.reduce<Record<string, HolidayItem[]>>((map, h) => {
      ...
      <div className="container mx-auto px-6 py-6 space-y-6">
        ...
        <div className="flex items-center justify-between gap-2">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-medium bg-white border text-muted-foreground hover:text-foreground transition-colors"
          >
            {sortDesc ? (
            ...
            {months.map((month) => (
              <div key={month} className="bg-white rounded-xl border overflow-hidden">
                <div className="px-5 py-3 border-b bg-slate-50/50 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <h2 className="font-semibold text-sm">{formatMonthLabel(month)}</h2>
                    {month === thisMonth && (
